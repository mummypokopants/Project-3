...
app.get('/scrape', function(req, res){
    ...
    // for (i=1;i<879;i++){ //<879
    var url = ('https://spring.ura.gov.sg/lad/ecas/motorist/pp_info/ppInfoDisplay.cfm');
    console.log(url);
      //requiring the objects.json
      var array = require('./objects.json');
      var result = [];

      //looping through the values inside objects.json
      for(var i=700; i<819; i++){
        // using jsonobject within the scope of the for loop
         let jsonobject = {};

        console.log(array[i].value)
        request.post(url,{form:{select_pp_name: array[i].value}}, function(error, response, html){
            if(!error){
        ...
        // console.log("This is the " + jsonobject.rates);
    ...
    setTimeout(function(){
      ...
      fs.writeFile('outputUra5.json', JSON.stringify(result), function(err){
